...
export default class BaiTapBookingTicket extends Component {
  state = {
    danhSachGheDaDat: [], // Danh sách ghế đã đặt
  };

  // Hàm xử lý khi chọn ghế
  chonGhe = (ghe) => {
    const { danhSachGheDaDat } = this.state;
    // Kiểm tra xem ghế đã tồn tại trong danh sách chưa
    const index = danhSachGheDaDat.findIndex((item) => item.soGhe === ghe.soGhe);
    if (index === -1) {
      // Nếu chưa tồn tại thì thêm vào danh sách
      danhSachGheDaDat.push(ghe);
      this.setState({ danhSachGheDaDat });
    }
  };

  // Hàm xử lý khi xóa ghế
  xoaGhe = (soGhe) => {
    const { danhSachGheDaDat } = this.state;
    // Lọc ra các ghế khác số ghế cần xóa
    const updatedGheDaDat = danhSachGheDaDat.filter((ghe) => ghe.soGhe !== soGhe);
    this.setState({ danhSachGheDaDat: updatedGheDaDat });
  };
      ...
      return (
        <div key={index}>
          <HangGhe
            ...
            danhSachGheDaDat={danhSachGheDaDat} // Truyền danh sách ghế đã đặt vào
          />
        </div>
      );
    });
        ...
        <div style={{ position: 'fixed', width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.8)' }}>
          <div className="row text-center">
            <div className="col-md-8 order-md-1 order-2">
              <h1 className='text-light mt-5'> Bài Tập Đặt Vé CYBER</h1>
              <div className='mt-5 text-light'> Màn hình </div>
              <div className='mt-2' style={{ display: 'flex', justifyContent: 'center', flexDirection: 'row' }}>
                <div className='screen'></div>
              </div>
              {this.renderHangGhe()}
            </div>
            <div className="col-md-4 order-md-2 order-1 text-center">
              <h1 style={{ fontSize: '35px' }} className='display-4 text-light mt-2'> Danh Sách Ghế Bạn Chọn </h1>
              <ThongTinDatGhe danhSachGheDaDat={danhSachGheDaDat} xoaGhe={this.xoaGhe} />
            </div>
          </div>
        </div>
      </div>
